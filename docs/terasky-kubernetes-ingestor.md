...
2. **Add to backend** (`packages/backend/src/index.ts`) using New Backend System (v1.42.0+):
```typescript
// New Backend System - automatic plugin registration
...
```yaml
# New Backend System (v1.42.0+) requires serviceLocatorMethod
kubernetes:
  serviceLocatorMethod:
    type: 'multiTenant'        # NEW: Required for v1.42.0+
